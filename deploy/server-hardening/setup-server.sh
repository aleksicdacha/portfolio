...
echo ""
echo -e "${YELLOW}NEXT STEP — Install security monitoring (STRONGLY recommended):${NC}"
echo "  1. scp deploy/server-hardening/setup-security-monitoring.sh  root@$SERVER_IP:/root/"
echo "  2. scp deploy/server-hardening/daily-security-scan.sh         root@$SERVER_IP:/root/"
echo "  3. scp deploy/server-hardening/msmtp.conf.template            root@$SERVER_IP:/root/msmtp.conf"
echo "  4. ssh root@$SERVER_IP 'nano /root/msmtp.conf'   # fill in Gmail/SMTP credentials"
echo "  5. ssh root@$SERVER_IP 'nano /root/setup-security-monitoring.sh' # set ALERT_EMAIL"
echo "  6. ssh root@$SERVER_IP 'bash /root/setup-security-monitoring.sh 2>&1 | tee /root/security-setup.log'"
echo ""
echo "  This installs: ClamAV, rkhunter, chkrootkit, Lynis, AIDE, auditd"
echo "  + daily cron scan at 03:30 with email alerts"
