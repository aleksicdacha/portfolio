...
@Component({
    ...
    <div class="page-experience">
      ...
      <section class="page-hero section-sm">
        <div class="container">
          ...
          <p class="page-lead">Two decades of building for the web — from freelance projects in Nis to enterprise platforms in Switzerland and Denmark.</p>
        </div>
      </section>
      ...
      <section class="section" aria-label="Work experience timeline">
        <div class="container">
          <div class="timeline">
            @for (item of experience; track item.company + item.startDate; let i = $index) {
              <article class="timeline-item" [class.first]="i === 0">
                ...
                <div class="timeline-connector" aria-hidden="true">
                  <div class="connector-dot" [class.current]="item.endDate === 'Present'"></div>
                ...
                <div class="timeline-content">
                  <header class="timeline-header">
                    ...
                    <div class="timeline-company">
                      @if (item.companyUrl) {
                        <a [href]="item.companyUrl" target="_blank" rel="noopener noreferrer" class="company-link">
                          {{ item.company }}
                        </a>
                      } @else {
  ...
  styles: [`
    ...
    .page-lead {
      ...
      max-width: 600px; line-height: 1.7; margin-top: var(--space-4);
    }
    ...
    .connector-dot {
      ...
      &.current { border-color: var(--color-accent); background: var(--color-accent); }
    }
    .connector-line {
      flex: 1;
      width: 1px;
    ...
    .timeline-header { margin-bottom: var(--space-4); }
    .timeline-role {
      font-size: 1.25rem;
      font-weight: 600;
      color: var(--color-text);
    ...
    .company-location { color: var(--color-text-tertiary); }
    .timeline-summary {
      font-size: 0.9375rem;
      color: var(--color-text-secondary);
      line-height: 1.6;
    ...
    .timeline-tech { display: flex; flex-wrap: wrap; gap: var(--space-2); }

    /* Education */
    .section-alt { background: var(--color-surface); }
    .edu-grid { display: flex; flex-direction: column; gap: var(--space-4); margin-top: var(--space-8); }
    .edu-card {
      padding: var(--space-6);
    ...
    .edu-degree { font-size: 1rem; font-weight: 600; margin: 0 0 var(--space-2); color: var(--color-text); }
    .edu-institution { font-size: 0.9375rem; color: var(--color-text-secondary); margin: 0 0 var(--space-2); }
    .edu-years { font-size: 0.8125rem; color: var(--color-text-tertiary); margin: 0; }

    /* Brutalist variant */
    :host-context([data-variant="brutalist"]) .connector-dot { border-radius: 0; }
    ...
    @media (max-width: 768px) {
      .timeline-item { grid-template-columns: 80px 32px 1fr; gap: 0 var(--space-3); }
      .timeline-dates { font-size: 0.75rem; }
      .date-separator { display: none; }
    }
    @media (max-width: 520px) {
      .timeline-item { grid-template-columns: 1fr; }
      .timeline-dates { flex-direction: row; align-items: center; justify-content: flex-start; padding: 0; gap: var(--space-3); }
      .timeline-connector { display: none; }
      .date-separator { display: inline; }
    }
  `],
})
